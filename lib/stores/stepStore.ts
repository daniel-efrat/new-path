...
interface StepState {
  ...
  hollandResults?: { riasec_vector: Record<string, number>, riasec_code: string } | null
  setStepCompletion: (stepId: number, isCompleted: boolean) => Promise<void>
  initializeSteps: () => void
  resetSteps: () => void
  resetFromStep: (stepId: number) => void
  setUserId: (userId?: string) => void
  setHollandResults: (results: { riasec_vector: Record<string, number>, riasec_code: string } | null) => void
      ...
      fetchUserProgress: async (userId) => {
        ...
        try {
          ...
          if (data && data.steps_progress) {
            ...
            if (insertError) {
              console.error('Failed to create initial user progress:', insertError);
            }
          } else if (error) {
            throw error;
          }
      ...
      onRehydrateStorage: () => (state) => {
        ...
        setTimeout(async () => {
          try {
            ...
            if (currentUserId && hydratedUserId && hydratedUserId !== currentUserId) {
