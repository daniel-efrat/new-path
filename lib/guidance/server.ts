...
function buildGuidancePrompt(input: GuidanceInput) {
  ...
  const systemPrompt = [
    ...
    "כתוב למשתמש/ת מפת כיוון ראשונית וקצרה לאחר שאלון 1 בלבד.",
    "אסור להציג אחוזי התאמה למקצועות, אבחון סופי, ציוני יכולת, או המלצות למקצועות ספציפיים.",
    "מותר להציע כיוונים רחבים לבדיקה בשאלון 2, ולבסס אותם רק על הנתונים שסופקו.",
    "הפלט חייב להיות JSON תקין בלבד, בלי Markdown, לפי הסכמה.",
  ].join("\n");

  const userPrompt = [
    "נתוני שאלון 1 ליצירת דוח JSON:",
